...
uint16_t W25QXX1_ReadID(void)
{
    uint16_t Temp = 0;
    W25QXX_CS1 = 0;
    send_byte_to_flash(0x90);//发送读取ID命令
...
uint8_t W25QXX1_ReadSR(uint8_t regno)
{
    ...
    W25QXX_CS1 = 0;                          //使能器件
    send_byte_to_flash(command);            //发送读取状态寄存器命令
    byte = read_byte_from_flash(0Xff);        //读取一个字节
    W25QXX_CS1 = 1;                          //取消片选
    return byte;
}
...
void W25QXX1_Write_SR(uint8_t regno, uint8_t sr)
{
    ...
    W25QXX_CS1 = 0;                          //使能器件
    send_byte_to_flash(command);            //发送写取状态寄存器命令
    send_byte_to_flash(sr);                 //写入一个字节
    W25QXX_CS1 = 1;                          //取消片选
}
...
{
    W25QXX_CS1 = 0;                          //使能器件
    send_byte_to_flash(W25X_WriteEnable);   //发送写使能
    W25QXX_CS1 = 1;                          //取消片选
}
...
{
    W25QXX_CS1 = 0;                          //使能器件
    send_byte_to_flash(W25X_WriteDisable);  //发送写禁止指令
    W25QXX_CS1 = 1;                          //取消片选
}
...
void W25QXX1_Read(uint8_t *pBuffer, uint32_t ReadAddr, uint16_t NumByteToRead)
{
    ...
    send_byte_to_flash((uint8_t)((ReadAddr) >> 16)); //发送24bit地址
...
void W25QXX1_Write_Page(uint8_t *pBuffer, uint32_t WriteAddr, uint16_t NumByteToWrite)
{
    ...
    send_byte_to_flash((uint8_t)((WriteAddr) >> 16)); //发送24bit地址
...
void W25QXX1_Write(uint8_t *pBuffer, uint32_t WriteAddr, uint16_t NumByteToWrite)
{
    ...
    while(1)
    {
        W25QXX1_Read(W25QXX_BUF, secpos * 4096, 4096); //读出整个扇区的内容
        ...
        if(i < secremain) //需要擦除
        {
            W25QXX1_Erase_Sector(secpos);//擦除这个扇区
            for(i = 0; i < secremain; i++)	 //复制
            {
                W25QXX_BUF[i + secoff] = pBuffer[i];
            }
            W25QXX1_Write_NoCheck(W25QXX_BUF, secpos * 4096, 4096); //写入整个扇区

        }
        else W25QXX1_Write_NoCheck(pBuffer, WriteAddr, secremain); //写已经擦除了的,直接写入扇区剩余区间.
...
void W25QXX2_Write(uint8_t *pBuffer, uint32_t WriteAddr, uint16_t NumByteToWrite)
{
    ...
    while(1)
    {
        W25QXX2_Read(W25QXX_BUF, secpos * 4096, 4096); //读出整个扇区的内容
        for(i = 0; i < secremain; i++) //校验数据
        {
            if(W25QXX_BUF[secoff + i] != 0XFF)break; //需要擦除
        }
        if(i < secremain) //需要擦除
        {
            W25QXX2_Erase_Sector(secpos);//擦除这个扇区
            for(i = 0; i < secremain; i++)	 //复制
            {
                W25QXX_BUF[i + secoff] = pBuffer[i];
            }
            W25QXX2_Write_NoCheck(W25QXX_BUF, secpos * 4096, 4096); //写入整个扇区

        }
        else W25QXX2_Write_NoCheck(pBuffer, WriteAddr, secremain); //写已经擦除了的,直接写入扇区剩余区间.
        if(NumByteToWrite == secremain)break; //写入结束了
        else//写入未结束
        {
            secpos++;//扇区地址增1
...
void W25QXX1_Erase_Chip(void)
{
    ...
    W25QXX1_Wait_Busy();
    W25QXX_CS1 = 0;                          //使能器件
    send_byte_to_flash(W25X_ChipErase);        //发送片擦除命令
    W25QXX_CS1 = 1;                          //取消片选
    W25QXX1_Wait_Busy();   				   //等待芯片擦除结束
}
...
void W25QXX1_Erase_Sector(uint32_t Dst_Addr)
{
    ...
    send_byte_to_flash((uint8_t)((Dst_Addr) >> 16)); //发送24bit地址
    send_byte_to_flash((uint8_t)((Dst_Addr) >> 8));
    send_byte_to_flash((uint8_t)Dst_Addr);
    W25QXX_CS1 = 1;                          //取消片选
    W25QXX1_Wait_Busy();   				    //等待擦除完成
}
...
void W25QXX2_Erase_Sector(uint32_t Dst_Addr)
{
    ...
    send_byte_to_flash((uint8_t)Dst_Addr);
    W25QXX_CS2 = 1;                          //取消片选
    W25QXX2_Wait_Busy();   				    //等待擦除完成
}
//等待空闲
void W25QXX1_Wait_Busy(void)
{
    while((W25QXX1_ReadSR(1) & 0x01) == 0x01); // 等待BUSY位清空
}
void W25QXX2_Wait_Busy(void)
{
    while((W25QXX2_ReadSR(1) & 0x01) == 0x01); // 等待BUSY位清空
}
//进入掉电模式
void W25QXX1_PowerDown(void)
{
    W25QXX_CS1 = 0;                          //使能器件
    send_byte_to_flash(W25X_PowerDown);     //发送掉电命令
    W25QXX_CS1 = 1;                          //取消片选
    //delay_us(3);                            //等待TPD
}
