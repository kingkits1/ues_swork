...
{   
	...
	if(DMA2_Stream3->CR&(1<<19))
	{
		...
		{
		    wav_buffill(wave_play_buf1,4096,wavctrl.bps);
		}
		else
		{
		    // Stop DMA
	...
	}else 
	{
		...
		{
		    wav_buffill(wave_play_buf2,4096,wavctrl.bps);
		}
		else
		{
		    // Stop DMA
...
uint8_t wav_play_song(ENUM_WAVE_TYPES type)
{
	...
	if(res==0)//解析文件成功
	{
        ...
        // <===
		wavwitchbuf=0;
		fillnum=wav_buffill(wave_play_buf1,WAV_SAI_TX_DMA_BUFSIZE,wavctrl.bps);
        wavwitchbuf=1;
		fillnum=wav_buffill(wave_play_buf2,WAV_SAI_TX_DMA_BUFSIZE,wavctrl.bps);
		SAI_Play_Start();  
	}
	return res;
} 
